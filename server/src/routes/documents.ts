...
const bulkJobs: Record<string, { total: number; done: number; aiDone: number; docIds: number[] }> = {};
...
async function processBulkAI(jobId: string) {
  ...
  try {
    ...
    const rows = await db.all('SELECT DISTINCT ai_classification FROM documents WHERE ai_classification IS NOT NULL AND ai_classification != ""');
    await db.close();
  ...
  for (const docId of job.docIds) {
    try {
      ...
      if (doc && doc.indexed_text && doc.indexed_text.trim()) {
        await fetch(`${process.env.API_URL || 'http://localhost:3001/api'}/documents/${docId}/analyze/ai`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(categories.length > 0 ? { categories } : undefined),
        });
      }
      job.aiDone++;
    } catch {
...
router.post('/bulk-upload', upload.array('files', 100), async (req: Request, res: Response) => {
  ...
  for (const file of files) {
    ...
    try {
      if ((file.mimetype === 'image/jpeg' || file.mimetype === 'image/png') && ocrTexts[file.originalname]) {
...
router.get('/folders', async (req: Request, res: Response) => {
  try {
    const db = await getDb();
    const rows = await db.all('SELECT DISTINCT folder FROM documents WHERE folder IS NOT NULL AND folder != "" ORDER BY folder ASC');
    await db.close();
    const folders = rows.map((r: any) => r.folder);
    res.json(folders);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch folders', details: (err as Error).message });
  }
});
...
router.get('/by-folder/:folder', async (req: Request, res: Response) => {
  ...
  try {
    const db = await getDb();
    const docs = await db.all('SELECT * FROM documents WHERE folder = ? ORDER BY upload_date DESC', folder);
    await db.close();
    res.json(docs);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch documents by folder', details: (err as Error).message });
  }
});
...
router.post('/search', async (req: Request, res: Response) => {
  const { query = '', category, fileType, dateFrom, dateTo, page = 1, pageSize = 10 } = req.body;
  let sql = 'SELECT * FROM documents WHERE 1=1';
  const params: any[] = [];
  if (query) {
    sql += ' AND (original_name LIKE ? OR extracted_text LIKE ? OR ai_classification LIKE ?)';
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to search documents', details: (err as Error).message });
  }
});
...
router.post('/:id/analyze', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (doc.mime_type === 'application/pdf') {
      ...
      ocrText = (result.data.text && result.data.text.trim()) ? result.data.text : 'No text found';
    } else {
      await db.close();
      return res.status(400).json({ error: 'OCR can only be re-run on images or PDFs' });
    }
...
// POST /api/documents/:id/analyze/handwritten (images and PDFs, LSTM_ONLY, SINGLE_BLOCK)
router.post('/:id/analyze/handwritten', async (req: Request, res: Response) => {
  ...
  try {
    ...
    let ocrText = '';
    if (doc.mime_type === 'application/pdf') {
      ocrText = await ocrPdf(doc.file_path, true);
    } else if (doc.mime_type === 'image/jpeg' || doc.mime_type === 'image/png') {
      const Tesseract = require('tesseract.js');
      const result = await Tesseract.recognize(doc.file_path, 'eng', {
        logger: () => {},
        tessedit_ocr_engine_mode: 1, // LSTM_ONLY
        tessedit_pageseg_mode: 3 // SINGLE_BLOCK
      });
      ocrText = (result.data.text && result.data.text.trim()) ? result.data.text : 'No text found';
    } else {
      await db.close();
      return res.status(400).json({ error: 'Handwritten OCR can only be re-run on images or PDFs' });
    }
    const indexedText = ocrText.toLowerCase().replace(/\s+/g, ' ').trim();
    await db.run('UPDATE documents SET extracted_text = ?, indexed_text = ? WHERE id = ?', ocrText, indexedText, docId);
    await db.close();
    ...
    res.status(500).json({ error: 'Failed to re-run handwritten OCR', details: (err as Error).message });
  }
});
...
router.patch('/:id/category', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await db.run('UPDATE documents SET ai_classification = ?, folder = ?, file_path = ? WHERE id = ?', category, newFolder, newFilePath, docId);
    await db.close();
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: 'Failed to update category/folder', details: (err as Error).message });
  }
});
