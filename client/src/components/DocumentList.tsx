...
const getFileUrl = (filename: string) =>
  `${process.env.REACT_APP_API_URL?.replace("/api", "") || "http://localhost:3001"}/files/${filename}`;
...
const DocumentList: React.FC<DocumentListProps> = ({ folder = "" }) => {
      ...
      try {
        ...
        if (search || fileType || dateFrom || dateTo || filter) {
          ...
          const res = await axios.post(
            `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/search`,
            {
              query: search,
              category:
                filter && filter !== "__uncategorized__" ? filter : undefined,
          ...
          const res = await axios.get(
            `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/by-folder/${encodeURIComponent(folder)}`,
          );
          docs = res.data;
  ...
  useEffect(() => {
    setPage(1);
  }, [search, fileType, dateFrom, dateTo, filter, folder]);
  ...
  useEffect(() => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/${realId}${endpoint}`,
      )
      .then(() => {
    ...
    analyzeDocumentAI(nextId)
      .then(async (res) => {
        ...
        if (res.classification) {
          ...
          await axios.patch(
            `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/${nextId}/category`,
          ...
          setDocuments((prev) =>
            prev.map((d) =>
              d.id === nextId
                ? { ...d, ai_classification: res.classification }
                : d,
            ),
          );
        }
  ...
  const classifications = Array.from(
    new Set(documents.map((d) => d.ai_classification).filter(Boolean)),
  );
  const hasUncategorized = documents.some(
    (d) => !d.ai_classification || d.ai_classification.trim() === "",
  );
  const filteredDocs =
    filter === "__uncategorized__"
      ? documents.filter(
          (d) => !d.ai_classification || d.ai_classification.trim() === "",
        )
      : filter
        ? documents.filter((d) => d.ai_classification === filter)
        : documents;
  ...
  const handleBulkAi = async () => {
    ...
    try {
      for (const id of selectedDocs) {
        // Trigger AI analysis and get result
        const { data } = await axios.post(
          `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/${id}/analyze/ai`,
          allCategories.length > 0 ? { categories: allCategories } : undefined,
        );

        const classification = data.classification;
        if (classification) {
          // Save classification back to document
          await axios.patch(
            `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/${id}/category`,
            { category: classification },
          );

          setDocuments((prev) =>
            prev.map((d) =>
              d.id === id ? { ...d, ai_classification: classification } : d,
            ),
          );
        }
      }
      toast.success("AI analysis completed for selected documents");
      setSelectedDocs([]);
    } catch {
  ...
  const handleCategorySave = async (doc: Document) => {
    try {
      await axios.patch(
        `${process.env.REACT_APP_API_URL || "http://localhost:3001/api"}/documents/${doc.id}/category`,
        { category: categoryInput },
      );
      setDocuments((prev) =>
        prev.map((d) =>
          d.id === doc.id ? { ...d, ai_classification: categoryInput } : d,
        ),
      );
  ...
  const fileTypes = Array.from(
    new Set(documents.map((d) => d.mime_type).filter(Boolean)),
  );
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-8">
      ...
      <div className="mb-2 text-xs text-gray-500 flex items-center gap-2">
        ...
        {searchDuration !== null && (
          <span>in {searchDuration.toFixed(0)} ms</span>
        )}
        {totalPages > 1 && (
          <>
            <span className="ml-4">
      ...
      {showBulkProgress && (
        <div
          className="mb-4 bg-blue-100 border border-blue-200 text-blue-800 px-4 py-3 rounded relative"
          role="alert"
        >
          <strong className="font-bold">Processing in background:</strong>
          <span className="block sm:inline">
            {" "}
            {bulkProgress.done}/{bulkProgress.total} documents
          </span>
          <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
            <svg
              ...
              viewBox="0 0 20 20"
            >
              <path d="M10 15a1 1 0 01-.707-.293l-4-4a1 1 0 111.414-1.414L9 12.586V3a1 1 0 112 0v9.586l3.293-3.293a1 1 0 011.414 1.414l-4 4A1 1 0 0110 15z" />
            </svg>
          </span>
        </div>
      )}
            ...
            <li key={doc.id} className="py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                {doc.mime_type === "application/pdf" && (
                  <span className="w-10 h-10 flex items-center justify-center bg-red-100 text-red-600 rounded font-bold">
                    PDF
                  </span>
                )}
                {doc.mime_type === "text/plain" && (
                  <span className="w-10 h-10 flex items-center justify-center bg-blue-100 text-blue-600 rounded font-bold">
                    TXT
                  </span>
                )}
                <div>
                  <div className="font-medium">
                    {highlight(doc.original_name, search)}
                  </div>
                  ...
                  <div className="text-xs mt-1">
                    ...
                    {categoryEditId === doc.id ? (
                      <>
                        ...
                        />
                        <button
                          className="text-blue-600 text-xs mr-1"
                          onClick={() => handleCategorySave(doc)}
                        >
                          Save
                        </button>
                        <button
                      ...
                      <>
                        {doc.ai_classification ? (
                          <span>
                            {highlight(doc.ai_classification, search)}
                          </span>
                        ) : (
                        ...
                        <button
                          ...
                          onClick={() => handleCategoryEdit(doc)}
                        >
                          Edit
                        </button>
                      </>
                  ...
                  {doc.mime_type.startsWith("image/") &&
                    (doc.image_width || doc.image_height) && (
                      <div className="text-xs text-gray-600">
                        Dimensions: {doc.image_width} x {doc.image_height} px
                      </div>
                    )}
                  {/* PDF metadata */}
                  {doc.mime_type === "application/pdf" &&
                  ...
                  {doc.donut_classification && (
                    <div className="text-xs text-pink-700 font-semibold mt-1">
                      Donut: {doc.donut_classification}
                    </div>
                  )}
                  {/* Status */}
                  {docStatus[doc.id] && (
                    <div
                      className={`text-xs mt-1 ${
                        docStatus[doc.id] === "ocr-processing"
                          ? "text-yellow-700"
                          : docStatus[doc.id] === "ocr-done"
                            ? "text-green-700"
                            : docStatus[doc.id] === "ocr-failed"
                              ? "text-red-700"
                              : docStatus[doc.id] === "ai-processing"
                                ? "text-yellow-700"
                                : docStatus[doc.id] === "ai-done"
                                  ? "text-green-700"
                                  : docStatus[doc.id] === "ai-failed"
                                    ? "text-red-700"
                                    : docStatus[doc.id] === "donut-processing"
                                      ? "text-yellow-700"
                                      : docStatus[doc.id] === "donut-done"
                                        ? "text-green-700"
                                        : docStatus[doc.id] === "donut-failed"
                                          ? "text-red-700"
                                          : "text-gray-500"
                      }`}
                    >
